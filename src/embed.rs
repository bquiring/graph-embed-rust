...
pub fn compute_radii(
    ...
    while assigned_count < n && !times.is_empty() {
...
pub fn normalizeCommunityEmbeddings(
    ...
    assert_eq!(nc, radii_Ac.len());
    assert_eq!(nc, PT.nrows());
    assert_eq!(n, PT.ncols());
    ...
    G: Fn(&CsrMatrix<f64>, usize, &mut Vec<Vec<f64>>, &Vec<Vec<f64>>, &CsrMatrix<f64>),
{
    ...
    //rand_elems.extend((0..1).map(|_| rng.sample(&dist)));
    // let mut coords_Ac: Grid<f64> = Grid::from_vec(1, 1, rand_elems);
    let mut coords_Ac: Vec<_> = vec![vec![0.0; 1]; 1];
    for i in 0..1 {
        for k in 0..1 {
